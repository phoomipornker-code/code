...
// =========================================================================
// Hardware
// Boost stays on the proven 50 kHz plant. Forward uses 67 kHz so the Nr=Np
// transformer is not driven into saturation (B ∝ D/fs).
// =========================================================================
...
const float UNDER_AC_VOLTAGE_CRIT = 100.0;
const unsigned long AC_COLLAPSE_CONFIRM_MS = 2000;
const int MAX_DUTY_FORWARD = 460;          // ~45% of 1023, Nr=Np reset limit
...
static inline void enterForwardCv(const char *why, float vRaw, float vFilt) {
    ...
    fwdCvEnterMs = 0;
    Serial.printf("[INFO] FORWARD CV (%s) at raw=%.2f filt=%.2f I=%.2fA\n",
                  why, vRaw, vFilt, (float)i_bat_charge_filt);
}
...
static inline const char* forwardModeLabel() {
    if (forwardMode == FWD_SOFTSTART) return "FWD SS";
    if (forwardMode == FWD_CC) return "FWD CC";
    if (forwardMode == FWD_CV) return "FWD CV";
    return "FWD DN";
}
...
void setup() {
    ...
    xTaskCreatePinnedToCore(TaskSampleData, "ADC_PWM_Task", 6144, NULL, 2, NULL, 0);
    ...
    for(;;) {
        ...
        if (xSemaphoreTake(i2c_Mutex, adcWait)) {
            ...
            bool ac_raw_glitch = (raw_mv_v1 < ADC_RAW_MIN_VALID_MV) &&
                                 (power_stage_active ||
                                  i_ac_mag > ADC_GLITCH_CURRENT_GATE_A ||
                                  i_bat_charge_filt > ADC_GLITCH_CURRENT_GATE_A);
            ...
            if ((solar_raw_glitch || ac_raw_glitch || bat_raw_glitch) &&
                ...
                Serial.printf("[WARN] ADC glitch filtered: PVraw=%.1fmV ACraw=%.1fmV BATraw=%.1fmV duty=%d\n",
                              raw_mv_v0, raw_mv_v1, raw_mv_v2, raw_duty);
            }
            ...
            if (!ovp_latched &&
                ...
                Serial.printf("[CRITICAL] BMS-OPEN/preempt at raw=%.2f filt=%.2f I=%.2fA step=%.2f. PWM off.\n",
                              (float)v_bat, (float)v_bat_filt, (float)i_bat_charge_filt, vbat_step);
            }
            if (!ovp_latched &&
            ...
            if (!ovp_latched &&
                ...
                if (v_bat >= HARD_OVP_TRIP_VOLTAGE) {
                    ...
                    Serial.printf("[CRITICAL] RUNAWAY-CUT at %.2fV (filt=%.2fV, duty=%d).\n",
                                  (float)v_bat, (float)v_bat_filt, raw_duty);
                } else {
                    ...
                    if (currentState == STATE_BOOST) {
                        ...
                        if (forwardMode != FWD_CV) {
                            enterForwardCv("runaway-soft-cut", v_bat, v_bat_filt);
                        }
                    }
                    Serial.printf("[WARN] Runaway soft-cut duty at %.2fV (filt=%.2fV).\n",
                                  (float)v_bat, (float)v_bat_filt);
                }
            }
            if (!ovp_latched &&
                system_ON &&
                ...
                if (v_bat >= HARD_OVP_TRIP_VOLTAGE) {
                    ...
                    Serial.printf("[CRITICAL] SPIKE-PRECUT at %.2fV (step=%.2fV, filt=%.2fV, duty=%d).\n",
                                  (float)v_bat, vbat_step, (float)v_bat_filt, raw_duty);
                } else {
                    ...
                    if (currentState == STATE_BOOST) {
                        ...
                        fwdCurrIntegrator = 0.0f;
                        if (forwardMode != FWD_CV) {
                            enterForwardCv("spike-soft-cut", v_bat, v_bat_filt);
                        }
                    }
                    Serial.printf("[WARN] Spike soft-cut duty at %.2fV (step=%.2fV).\n",
                                  (float)v_bat, vbat_step);
                }
            }
            if (ovp_latched &&
                ...
                Serial.printf("[INFO] OVP latch cleared at %.2fV (release=%.2fV).\n",
                              vbat_peak, HARD_OVP_RELEASE_VOLTAGE);
            }
        ...
        if (system_ON) {
            ...
            if (currentState == STATE_OFF) {
                if (v_solar >= MIN_PV_VOLTAGE) {
                    ...
                    Serial.println("[INFO] Enter FORWARD SoftStart->CC->CV PI control.");
                }
                else {
                    system_ON = false;
                }
                ...
                if (v_ac_in < MIN_AC_VOLTAGE) {
                    ...
                    if (now - ac_collapse_start_time >= AC_COLLAPSE_CONFIRM_MS) {
                        system_ON = false;
                        Serial.printf("[CRITICAL] AC collapsed below %.1fV for %lums. Auto-Shutdown.\n",
                                      MIN_AC_VOLTAGE, AC_COLLAPSE_CONFIRM_MS);
                    }
                } else {
                    ac_is_collapsing = false;
                }
        ...
        if (system_ON && currentState != STATE_OFF) {
            ...
            if (currentState == STATE_FORWARD) {
                ...
                if (v_ac_in < UNDER_AC_VOLTAGE_CRIT) {
                    ...
                    dDuty = boostClampf(dDuty, -dutyStepLimit, dutyStepLimit);
                    float dutyTarget = duty_accumulator + dDuty;
                    ...
                    if (v_bat > (v_bat_filt + 1.5f)) {
                        dutyTarget = boostMinf(dutyTarget, duty_accumulator - 5.0f);
                ...
                if (v_bat_filt >= BMS_PREEMPT_ZONE_V || v_bat >= BMS_PREEMPT_ZONE_V) {
                    if (duty_accumulator > FWD_BMS_PREEMPT_DUTY_CAP_RAW) {
                        duty_accumulator = FWD_BMS_PREEMPT_DUTY_CAP_RAW;
                    }
                    if (v_bat_filt >= TARGET_CV_VOLTAGE) {
                        duty_accumulator = boostMinf(duty_accumulator, 80.0f);
                    }
                }
                ...
                if (v_solar <= 0.0f) {
                    ...
                    if (v_bat_filt >= BOOST_CV_FORCE_VOLTAGE ||
                        boostMaxf((float)v_bat, (float)v_bat_filt) >= BOOST_CV_FORCE_VOLTAGE) {
                        boostNewMode = BOOST_NEW_CV;
                        boostNewCurrIntegrator = 0.0f;
                        boostNewVoltIntegrator = 0.0f;
                    ...
                    if (v_bat > (v_bat_filt + 1.5f)) {
                        dutyTarget = boostMinf(dutyTarget, duty_accumulator - 6.0f);
                        dutyTarget = boostMaxf(0.0f, dutyTarget);
                    }
                    dutyTarget = boostClampf(dutyTarget, 0.0f, (float)allowed_max_duty);
        ...
        if (now - last_lcd_refresh >= LCD_REFRESH_INTERVAL_MS) {
            if (xSemaphoreTake(i2c_Mutex, 50)) {
                if (system_ON && charge_full_hold) {
                    ...
                    lcdPrintLineFmt(1, "BAT:%5.1fV I:%4.2fA", (float)v_bat_filt, (float)i_bat_filt);
                    ...
                    if (currentState == STATE_BOOST) {
                        lcdPrintLineFmt(1, "%-8s PWR:%5.1fWh", "BOOST PV", total_Wh);
                        lcdPrintLineFmt(2, "IN :%5.1fV %5.1fA", (float)v_solar, (float)i_solar);
                    } else {
                        lcdPrintLineFmt(1, "%-8s PWR:%5.1fWh", forwardModeLabel(), total_Wh);
                        lcdPrintLineFmt(2, "IN :%5.1fV %5.1fA", (float)v_ac_in, (float)i_ac_in);
                    }
                    lcdPrintLineFmt(3, "OUT:%5.1fV %5.1fA", (float)v_bat, (float)i_bat);
                } else if (ovp_latched || show_ovp_alert) {
                    lcdPrintLineRaw(0, "OVP TRIPPED");
                    lcdPrintLineFmt(1, "VBAT:%5.1fV T:%4.1f", (float)v_bat_filt, (float)ovp_trip_voltage);
                    ...
                    lcdPrintLineFmt(1, "PV :%5.1fV AC:%5.1fV", (float)v_solar, (float)v_ac_in);
                    lcdPrintLineFmt(2, "BATT:%5.1fV", (float)v_bat);
                    lcdPrintLineRaw(3, "");
                }
                xSemaphoreGive(i2c_Mutex);
                lcd_mutex_fail_count = 0;

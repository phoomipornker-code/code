...
/* The screenshot shows Duty = 0.41 while the voltage loop is in charge:
 * 0.1 * (58 - 53.9) = 0.41, which is below the current loop's 1.0 * (5 - 4.5). */
static void testOperatingPoint() {
  CVCCController c;
  c.begin();
  const CVCCOutput out = c.update(53.9f, 4.5f);
  expectNear("CV branch", out.cv, 0.41f);
  expectNear("CC branch", out.cc, 0.5f);
  expectNear("duty follows the smaller branch", out.duty, 0.41f);
  expectTrue("voltage loop reported as active", !c.currentLimited);
}

/* Pull more current than the limit and the current branch must take over. */
static void testCurrentLimitTakesOver() {
  CVCCController c;
  c.begin();
  const CVCCOutput out = c.update(50.0f, 4.9f);
  expectNear("CV branch", out.cv, 0.8f);
...
/* Above the setpoint both branches go negative and the output saturation has to
 * floor the duty cycle at zero rather than let it wrap or go negative. */
static void testOvervoltageFloorsDuty() {
  CVCCController c;
  c.begin();
  const CVCCOutput out = c.update(62.0f, 0.0f);
  expectNear("CV branch is negative", out.cv, -0.4f);
  expectNear("duty clamped to DUTY_MIN", out.duty, 0.0f);
}
...
static void testForwardEulerDelay() {
  PIController pi{0.0f, 2.0f, 0.001f, -1.0f, 1.0f, -1.0f, 1.0f, 0.0f};
...
static void testProportionalSaturation() {
  PIController pi{1.0f, 0.0f, 0.001f, -1.0f, 1.0f, -1.0f, 1.0f, 0.0f};
  expectNear("large positive error clamped", pi.update(40.0f), 1.0f);
  expectNear("large negative error clamped", pi.update(-40.0f), -1.0f);
}

static void testIntegratorAntiWindup() {
  PIController pi{0.0f, 100.0f, 0.001f, -1.0f, 1.0f, -1.0f, 1.0f, 0.0f};
  for (int step = 0; step < 200; ++step) {
    pi.update(1.0f);
  }
  expectNear("integrator state clamped at I_SAT_MAX", pi.iState, 1.0f);
  expectNear("output clamped too", pi.update(1.0f), 1.0f);

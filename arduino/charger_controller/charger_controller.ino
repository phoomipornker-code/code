...
const float UNDER_PV_VOLTAGE_CRIT = 38.0;  // ต่ำกว่า 39V เกิน 2 วินาที สั่งตัด
...
void TaskSampleData(void * pvParameters) {
    ...
    for(;;) {
        if (xSemaphoreTake(i2c_Mutex, 50)) {
            ...
            if (i_solar < NOISE_I_THRESHOLD) i_solar = 0.0;
            if (i_ac_in < NOISE_I_THRESHOLD) i_ac_in = 0.0;
            if (i_bat   < NOISE_I_THRESHOLD) i_bat   = 0.0;
            ...
            if (currentState == STATE_OFF) {
                ...
                if (v_solar < UNDER_PV_VOLTAGE_CRIT) {
                    if (!pv_is_collapsing) {
                        ...
                        pv_collapse_start_time = millis();
                    }

                    if (millis() - pv_collapse_start_time >= 2000) {
                        system_ON = false;
        ...
        if (system_ON && currentState != STATE_OFF) {
            ...
            total_Wh += ((v_bat * i_bat) * (millis() - last_millis)) / 3600000.0;
        } else {
            ledcWrite(PWM_FORWARD_PIN, 0);
            ledcWrite(PWM_BOOST_PIN, 0);
        }
...
void TaskLCDLoop(void * pvParameters) {
    ...
    for(;;) {
        if (millis() - last_lcd_recover > 2000) {
            last_lcd_recover = millis();
            if (xSemaphoreTake(i2c_Mutex, 20)) {
                lcd.init();
                lcd.backlight();
                xSemaphoreGive(i2c_Mutex);
            }
        ...
        bool current_start = digitalRead(BUTTON_START_PIN);
        bool current_stop  = digitalRead(BUTTON_STOP_PIN);

        if (current_start == LOW && last_start_state == HIGH) {
            if (v_solar >= MIN_PV_VOLTAGE || v_ac_in >= MIN_AC_VOLTAGE) {
                system_ON = true;
        ...
        if (system_ON != last_system_state) {
            if (xSemaphoreTake(i2c_Mutex, 50)) { lcd.clear(); xSemaphoreGive(i2c_Mutex); }
            last_system_state = system_ON;
        }

        if (show_no_power_alert && (millis() - alert_millis > 3000)) {
            show_no_power_alert = false;
            if (xSemaphoreTake(i2c_Mutex, 50)) { lcd.clear(); xSemaphoreGive(i2c_Mutex); }
        }

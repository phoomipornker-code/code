...
const float CAL_SCALE_V_BAT   = 41.85; // proven Boost v14-cv-stable
...
const float BOOST_CV_EXIT_VOLTAGE  = 54.20;   // was 54.80 — hunting near 54.8 started exit timer
const float BOOST_CC_TAPER_START_V = 54.80;
// Boost BMS-OPEN (exact cv58-stability-v14-cv-stable)
const float BOOST_BMS_OPEN_DETECT_V = 56.30f;
const float BOOST_BMS_OPEN_CURRENT_MAX_A = 1.20f;
// Forward BMS-OPEN (hardened — separate from Boost)
const float FWD_BMS_OPEN_DETECT_V = 56.80f;
const float FWD_BMS_OPEN_CURRENT_MAX_A = 0.25f;
const float BMS_OPEN_JUMP_DELTA_V = 1.2;
const float BMS_PREEMPT_DUTY_CAP_RAW = 140.0;
const float BMS_PREEMPT_ZONE_V = 55.95;
const float BMS_OPEN_NEAR_FULL_MAX_V = 56.60f; // Forward only: near-full taper ≠ open
const float BOOST_CV_IREF_SLEW_A = 0.04;      // slower Iref (was 0.08) — less I chatter
const float BOOST_CV_NEAR_BAND_V = 0.80;      // gentle earlier (was 0.35)
const float BOOST_CV_DUTY_STEP_NEAR = 0.5;    // was 0.8
const float BOOST_CV_DUTY_STEP_FAR = 1.2;     // was 2.5 — stop duty slam when far
const float BOOST_CV_PAVAIL_MARGIN = 0.85f;   // CV-only: cap iReq to ~85% of filtered Ppv
const float BOOST_CV_MID_START_V = 55.00f;   // below target: progressive iReq cap
    ...
    for(;;) {
        ...
        if (sample_ok) {
            ...
            if (!ovp_latched &&
                ...
                ((v_bat >= BOOST_BMS_OPEN_DETECT_V) ||
                 (vbat_step >= BMS_OPEN_JUMP_DELTA_V) ||
                 (v_bat > (v_bat_filt + 1.8f)) ||
                 (v_bat_filt >= BOOST_BMS_OPEN_DETECT_V &&
                  i_bat_charge_filt <= BOOST_BMS_OPEN_CURRENT_MAX_A))) {
                ovp_latched = true;
                ovp_trip_voltage = max(v_bat, v_bat_filt);
                ovp_trip_ms = now;
                forceSafeShutdown();
                charge_full_hold = true;  // v14: treat as full-hold after BMS open
            ...
            // ---- Forward BMS-OPEN: hardened (confirm + near-full ignore) ----
            else if (!ovp_latched && system_ON && currentState == STATE_FORWARD && raw_duty > 0) {
                bool bms_zone = (v_bat_filt >= BMS_PREEMPT_ZONE_V || v_bat >= BMS_PREEMPT_ZONE_V);
                bool i_collapsed = (i_bat_charge_filt <= FWD_BMS_OPEN_CURRENT_MAX_A) &&
                                   (i_bat_charge_abs <= (FWD_BMS_OPEN_CURRENT_MAX_A + 0.15f));
                bool bms_v_jump =
                    (vbat_step >= BMS_OPEN_JUMP_DELTA_V) ||
                    (v_bat > (v_bat_filt + 1.8f));
                bool bms_v_high =
                    (v_bat >= FWD_BMS_OPEN_DETECT_V) &&
                    (v_bat_filt >= (FWD_BMS_OPEN_DETECT_V - 0.25f));
                bool near_full_taper =
                    (v_bat_filt <= BMS_OPEN_NEAR_FULL_MAX_V) &&
                    (v_bat <= (BMS_OPEN_NEAR_FULL_MAX_V + 0.30f)) &&
                    (i_bat_charge_filt > FWD_BMS_OPEN_CURRENT_MAX_A);
                bool bms_suspect = bms_zone && (bms_v_jump || bms_v_high) && !near_full_taper;
            ...
            // HARD OVP: Boost = immediate (v14); Forward = confirm + pack window
            if (!ovp_latched &&
            ...
            // Boost runaway soft-cut — exact v14 (Boost only)
            if (!ovp_latched &&
                system_ON &&
                currentState == STATE_BOOST &&
                raw_duty > 0 &&
            ...
            // Forward runaway soft-cut (hardened — no false OVP from lone raw spike)
            if (!ovp_latched &&
                system_ON &&
            ...
            // Forward spike pre-cut (hardened)
            if (!ovp_latched &&
                system_ON &&
                currentState == STATE_FORWARD &&
                raw_duty > 0 &&
        ...
        if (system_ON && currentState != STATE_OFF) {
            ...
            if (currentState == STATE_FORWARD) {
                ...
                if (v_solar <= 0.0f) {
                    ...
                    if (nearTarget) {
                        float nearCap = 0.8f + boostClampf(vErr / BOOST_CV_NEAR_BAND_V, 0.0f, 1.0f) * 1.0f;
                        if (iReq > nearCap) iReq = nearCap;
                    }
                    // Mid approach (field log ~55.0–55.5): do not demand 3.5 A into weak PV.
                    if (v_bat_filt >= BOOST_CV_MID_START_V) {
                        float rem = BOOST_CV_TARGET_VOLTAGE - v_bat_filt;
                        float midCap = 0.5f + boostClampf(rem / 1.0f, 0.0f, 1.0f) * 1.5f; // 0.5..2.0 A
                        if (iReq > midCap) iReq = midCap;
                    }
                    ...
                    boostNewIrefCvCmd = boostApplySlew(iReq, boostNewIrefCvCmd,
                                                       BOOST_CV_IREF_SLEW_A, BOOST_CV_IREF_SLEW_A * 1.5f);
                    float iRef = boostNewIrefCvCmd;
                    float iErr = iRef - i_bat_charge_filt;
                    float dDuty = boostRunPI(iErr, BOOST_CURR_KP * (nearTarget ? 0.40f : 0.65f),
                    ...
                    dutyTarget = boostClampf(dutyTarget, 0.0f, (float)allowed_max_duty);
                    float cvSlewUp = nearTarget ? 0.8f : 1.5f;
                    float cvSlewDown = nearTarget ? 1.5f : 3.0f;

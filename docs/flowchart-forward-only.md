...
| รายการ | ค่า | ความหมาย |
|--------|-----|----------|
| ความถี่ PWM | 50 kHz | ความเร็วสวิตช์ MOSFET Forward |
| ขา PWM | GPIO 14 | ขับเกตฝั่ง Forward |
| กระแสเป้า (CC) | 6.0 A | ชาร์จกระแสคงที่ |
| แรงดันเป้า (CV) | 56.0 V | ชาร์จแรงดันคงที่ |
| Duty สูงสุด | 490 / 1023 (~47.9%) | แนะนำ ≤ 460 ถ้าหม้อแปลงมีขด Nr |
...
flowchart TD
    ...
    G -- ไม่ได้พัก --> K["⑩ คำนวณ PID กระแส<br/>ความผิดพลาด = 6.0 − กระแสแบต"]
    K --> L["⑪ คำนวณ PID แรงดัน<br/>ความผิดพลาด = 56.0 − แรงดันแบต<br/>ถ้าใกล้เป้ามาก ±0.12V ถือว่าถึงแล้ว"]
    L --> M["⑫ เลือกคำสั่งที่น้อยกว่า<br/>ระหว่างผล PID กระแส กับ PID แรงดัน<br/>แล้วจำกัดไม่ให้ปรับแรงเกินไป"]
    M --> N["⑬ บวกคำสั่งเข้า duty สะสม<br/>แล้วจำกัดไม่เกิน 490"]
    N --> O["⑭ ส่ง PWM ออกขา GPIO 14<br/>ความถี่ 50 กิโลเฮิรตซ์"]
...
เป้าคือดันกระแสให้เข้าใกล้ **6 แอมป์**
...
duty = duty + final
\]
แล้วบังคับให้อยู่ในช่วง **0 ถึง 490**  
(ถ้าใช้หม้อแปลงรีเซ็ต Nr = Np แนะนำเพดาน **460**)

### ⑭ เขียน PWM ออกขา Forward
ส่งค่า duty ไปที่ **GPIO 14** ความถี่ **50 kHz** เพื่อขับ MOSFET ฟอร์เวิร์ด
...
⑦–⑨ ถ้า FULL HOLD → รอจน Vbat ≤ 54V แล้วค่อยชาร์จใหม่
     ↓
⑩ e_cc = 6.0 − Ibat     → pid_cc
⑪ e_cv = 56.0 − Vbat    → pid_cv
⑫ final = min(pid_cc, pid_cv)
⑬ duty = clamp(duty + final, 0..490)
...
3. **PID กระแส 6A กับ PID แรงดัน 56V ทำพร้อมกัน**  
4. **ใช้คำสั่งที่น้อยกว่า** เพื่อคุมทั้งกระแสและแรงดัน  
5. **Duty ไม่เกิน 490** แล้วค่อยออก PWM  
